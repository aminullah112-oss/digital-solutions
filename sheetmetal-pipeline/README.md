...
## UI — spec builder and validator (browser-side only)

`ui/index.html` is a static page: a form mirroring the Excel schema, a
batch table, and an upload-and-preview flow for an existing workbook.

**What it actually does, said plainly because it's easy to overclaim this:**
it runs entirely in the browser, on GitHub Pages (static hosting — no
server). It cannot run FreeCAD and does not generate STEP or DXF files.
...
- Builds a real `.xlsx` with the correct columns via a vendored copy of
  ...
  — verified end-to-end during development: a file generated by this page,
  fed into the real batch script, built successfully.
...
Generating the actual part is still Stage 4/5's job: run
`freecadcmd scripts/run_batch.py` somewhere FreeCAD is installed (see
Stage 1's `setup_env.sh`). No browser can do that part — this page is a
front door to that pipeline, not a replacement for it.

**Why static-only, not a "real" generate button:** the FreeCAD environment
here is a ~1.5GB conda install, plus native OCCT geometry computation that
runs for actual wall-clock seconds per part — nothing GitHub Pages (or any
static host) can execute. Wiring up live 3D generation from a browser would
need a persistent backend server with FreeCAD installed, which is real
infrastructure this repo doesn't have (and this session, itself an
ephemeral container, can't stand up something durable either). If that's
wanted next, it's a distinct, larger piece of work — a small Flask/FastAPI
service wrapping `bracket_model.build_bracket`, deployed somewhere that
stays up (a VPS, Render, Fly.io — not a serverless function, the conda env
is too large for most free tiers' size limits).

Tested with Playwright/Chromium against a local static server before
considering this done: material dropdown populated and defaulted
correctly, a passing part shows the right K-factor and flat-length numbers,
a failing part (tight radius on thick gauge) shows both violated
manufacturability rules, "add to batch" and "download .xlsx" produce a
file `run_batch.py` actually accepts, and uploading the real
`input/parts_sample.xlsx` reproduces all 6 rows' pass/fail results
identically to the Python batch log above.

## Status

All 5 stages complete, plus a browser-side UI on top. Everything above ran
headless via `freecadcmd`, end to end: Excel row in, validated against real
manufacturability limits, STEP + DXF out, K-factor from a real (if
placeholder) material table rather than a guess. The UI adds a validated
front door to that same pipeline; it does not run any part of the pipeline
itself (see the UI section above for exactly where that line sits).
