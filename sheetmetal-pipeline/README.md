...
## UI — spec builder, validator, and (with a backend) real generation

`ui/index.html` is a static page: a form mirroring the Excel schema, a
batch table, an upload-and-preview flow for an existing workbook, and a
**Generate real part** button that's live once a backend is connected.

**On its own (no backend configured), it runs entirely in the browser** on
GitHub Pages (static hosting — no server) and cannot run FreeCAD:

- Runs the *exact same* manufacturability checks as
  `bracket_model.validate_params` (same thresholds, ported line for line
  into `ui/app.js`) and the *exact same* bend-allowance formula as
  `expected_flat_length` — not an approximation, the same math, so a part
  that validates here validates in the real pipeline too.
- Looks up K-factor from the live `config/kfactor_table.json` via `fetch()`
  (single source of truth — the page doesn't carry its own copy of the
  table that could drift from the one `run_batch.py` actually uses).
- Builds a real `.xlsx` with the correct columns via a vendored copy of
  [SheetJS](https://sheetjs.com/) (`ui/xlsx.full.min.js`), downloadable and
  directly usable as `freecadcmd scripts/run_batch.py your_download.xlsx`.
- Re-validates an uploaded `.xlsx` row-by-row client-side, so you can catch
  a bad row before spending time on the real (much slower) FreeCAD batch.

**With a backend URL entered and connected** (deploy one — see
`backend/README.md`), the same validated part can be sent straight to a
real server running the identical `build_bracket()` this pipeline has used
since Stage 2, and the browser downloads back a real STEP + DXF zip.
Verified end-to-end with Playwright: form filled in a real browser, backend
reachable over a genuinely different origin (exercising real CORS, not a
same-origin shortcut), click Generate, a real zip downloads containing a
valid STEP file and DXF.

Tested with Playwright/Chromium before considering either mode done:
material dropdown populated and defaulted correctly, a passing part shows
the right K-factor and flat-length numbers, a failing part (tight radius on
thick gauge) shows both violated manufacturability rules, "add to batch"
and "download .xlsx" produce a file `run_batch.py` actually accepts,
uploading the real `input/parts_sample.xlsx` reproduces all 6 rows'
pass/fail results identically to the Python batch log above, and (backend
mode) a live build request round-trips a real, valid STEP+DXF pair.

## Backend — actual live generation

`backend/` is a Flask + gunicorn API wrapping `bracket_model.build_bracket`
directly (running inside the same FreeCAD conda env Stage 1 built — its
Python bindings only import correctly with that env's lib directory on
`sys.path`, which a bare Python process has to set up itself since
`freecadcmd`'s launcher normally does it for you).

- `POST /api/build` — validates, builds, and returns a zip of
  `{job_id}_folded.step` + `{job_id}_flat.dxf`, or a 4xx with a clear error
  (bad JSON, missing field, unknown material, failed manufacturability
  check) — every failure mode Stage 3/4/5 established stays intact here.
- `GET /api/materials` — the live `config/kfactor_table.json`.
- `GET /health` — liveness + FreeCAD version, for monitoring and for the
  UI's "Check connection" button.
- Runs gunicorn with `sync` workers (not threads/async): each worker is a
  separate OS process with its own FreeCAD C++ state, which is what keeps
  concurrent requests from corrupting each other and contains a native
  crash on pathological geometry to just the one worker handling it.

Tested directly (gunicorn, 2 workers, this repo's dev environment):
`/health`, `/api/materials`, a successful build returning a real
27KB STEP + 7KB DXF, all four failure paths (bad JSON, missing fields,
unknown material, manufacturability violation) each returning the right
status code and message, and 4 concurrent build requests all completing
correctly with no cross-contamination between them.

**Deployment is a manual step you need to do** — see
[`backend/README.md`](backend/README.md) for the full walkthrough
(Oracle Cloud "Always Free" ARM VM, since FreeCAD's footprint rules out
free tiers on Render/Fly.io/Railway; Docker + Caddy for automatic HTTPS,
which is not optional — GitHub Pages serves the UI over HTTPS, and
browsers block `fetch()` from an HTTPS page to a plain HTTP endpoint by
default, so a bare-HTTP backend would silently fail from the live UI).
This session's own network policy blocks Docker Hub image pulls entirely,
so the Dockerfile/compose setup could not be built-and-run end-to-end here
— it follows the same install steps already proven working outside a
container (Stage 1's `setup_env.sh`), and a normal VM's unrestricted
internet access shouldn't hit that same wall, but treat first deployment
as the actual first real test of the container build itself.

## Status

All 5 stages complete, plus a UI and a real backend API on top. Everything
runs headless via `freecadcmd`, end to end: Excel row in (or a browser
form, via the backend), validated against real manufacturability limits,
STEP + DXF out, K-factor from a real (if placeholder) material table
rather than a guess. The one piece not yet live is deployment itself: the
backend is built and tested locally but needs to actually be deployed
(see `backend/README.md`) before the UI's "Generate real part" button has
anything to talk to.
