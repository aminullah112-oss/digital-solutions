...
const ROW1_TRIPLED = tripled(ROW1);
const ROW2_TRIPLED = tripled(ROW2);

const TILE_CLASS = 'w-[420px] h-[270px] rounded-2xl object-cover flex-shrink-0';
...
export default function MarqueeSection() {
  ...
  useEffect(() => {
    // Direct DOM writes (no React state) so a scroll tick never triggers a re-render —
    // this only ever needs to move a `transform`, which willChange already primes.
    const handleScroll = () => {
      ...
      <div className="flex flex-col gap-3">
        <div ref={row1Ref} className="flex gap-3" style={{ willChange: 'transform' }}>
          {ROW1_TRIPLED.map((src, i) => (
            <img key={`row1-${i}`} src={src} loading="lazy" alt="" className={TILE_CLASS} />
          ))}
        </div>
        <div ref={row2Ref} className="flex gap-3" style={{ willChange: 'transform' }}>
          {ROW2_TRIPLED.map((src, i) => (
            <img key={`row2-${i}`} src={src} loading="lazy" alt="" className={TILE_CLASS} />
          ))}
        </div>
      </div>
    </section>
